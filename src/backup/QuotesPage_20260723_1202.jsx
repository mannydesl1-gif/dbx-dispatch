    ...
    <style>
      ...
      @media print{
        /* Zero the page margin so the browser has no room to inject its own
           date / blob: URL header and footer. Real margins are applied below. */
        @page{size:letter;margin:0}
        /* Side + bottom margins on the body. The TOP margin is handled by
           .content's padding instead, because the fixed header occupies that
           space on every page. */
        body{padding:0 16mm 16mm;-webkit-print-color-adjust:exact;print-color-adjust:exact}
        ...
        /* Must clear the FULL rendered header height. The right-hand meta column
           (QUOTE title + 4 lines) is ~89px — taller than the 44px logo — so the
           header is ~39mm. Reserve 46mm to leave a clear gap under the red rule;
           if the meta block ever grows, raise this to match. */
        .content{padding-top:46mm}
