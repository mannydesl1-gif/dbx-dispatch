...
const S_LABEL = { unassigned:"Unassigned", assigned:"Assigned", "in-transit":"In Transit", "ready-to-bill":"Ready to Bill", closed:"Closed", "no-charge":"Closed – No Charge", invoiced:"Invoiced", cancelled:"Cancelled",
...
<!-- Header -->
<div style="display:grid;grid-template-columns:auto 1fr 1fr;gap:20px;align-items:center;border-bottom:4px solid #dc2626;padding-bottom:14px;margin-bottom:0">
...
<!-- BOL Info Bar -->
<div style="display:flex;justify-content:space-between;align-items:flex-start;background:#f8fafc;border:1.5px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;padding:14px 18px;margin-bottom:18px">
  <div>
  ...
  <div style="text-align:right;font-size:12px;line-height:2;color:#333">
    ...
    <div style="margin-top:6px;padding:4px 12px;background:#dc2626;color:#fff;border-radius:4px;font-size:11px;font-weight:700;display:inline-block">${billingDiv.name}</div>
  </div>
</div>
...
async function downloadBolPdf(o, divInfo, includePod=false, includePricing=false, driverIndex=0, client=null) {
  try {
    // Build the order payload the same way as the email functions
    const div = (typeof DIVISIONS !== "undefined" && DIVISIONS) ? DIVISIONS.find(d => d.id === o.divId) : null;
    const orderPayload = {
      ...o,
      divName: div?.name || divInfo?.name || ""
    };
    const clientPayload = client ? {
      name: client.name || "",
      street: client.street || "",
      city: client.city || "",
      provState: client.provState || "",
      postalZip: client.postalZip || "",
      country: client.country || "",
      email: client.billingEmail || client.email || ""
    } : null;

    // Get the downloadBolPdf URL — try config first, then derive from sendBolEmail URL
    let downloadUrl = CLOUD_FUNCTIONS && CLOUD_FUNCTIONS.downloadBolPdf;
    if (!downloadUrl && CLOUD_FUNCTIONS && CLOUD_FUNCTIONS.sendBolEmail) {
      // Firebase function URLs follow the pattern: https://<funcname-lowercase>-<suffix>.run.app
      downloadUrl = CLOUD_FUNCTIONS.sendBolEmail.replace(/sendbolemail/i, "downloadbolpdf");
    }
    if (!downloadUrl) {
      alert("Cloud Function URL not configured. Please add `downloadBolPdf` to CLOUD_FUNCTIONS in client.config.js");
      return;
    }
    ...
    // Verify we got a PDF back, not HTML (in case of redirect/error)
    const contentType = res.headers.get("content-type") || "";
    if (!contentType.includes("application/pdf")) {
      const errText = await res.text();
      alert(`PDF generation returned unexpected content. Check that the Cloud Function 'downloadBolPdf' is deployed.\n\nResponse: ${errText.substring(0,200)}`);
      return;
    }

    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const filename = `BOL_${o.bol}${includePricing ? "_with_pricing" : ""}.pdf`;
    // Open in new window so user can view and print/save
    ...
    // Clean up the object URL after a delay (give browser time to load it)
...
export default function App() {
  ...
  const nav = [{id:"dashboard",l:"Dashboard",i:"dash"},{id:"ol",l:"Orders",i:"file"},{id:"cr",l:"Live Crew",i:"users"},{id:"cl",l:"Clients",i:"users"},{id:"lo",l:"Locations",i:"map"},{id:"eq",l:"Equipment",i:"truck"},{id:"dr",l:"Drivers / Employees / Suppliers",i:"users"},{id:"pp",l:"PAPS / PARS",i:"barcode"},{id:"ts",l:"Timesheets",i:"calendar"},{id:"ev",l:"Events",i:"calendar"},{id:"qt",l:"Quotes",i:"file"},{id:"rp",l:"Reports",i:"chart"},{id:"sr",l:"Search",i:"search"},{id:"cd",l:"Documents",i:"file"},{id:"ed",l:"Employee Docs",i:"users"}];
...
function Dashboard({db, cnt, go, newOrd}) {
  ...
  return <div style={{padding:20}}>
    ...
    <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
      ...
      {[{k:"unassigned",l:"Unassigned",c:"#ef4444"},{k:"assigned",l:"Assigned",c:"#f59e0b"},{k:"in-transit",l:"In Transit",c:"#8b5cf6"},{k:"ready-to-bill",l:"Ready to Bill",c:"#f97316"},{k:"closed",l:"Closed",c:"#22c55e"},{k:"no-charge",l:"Closed – No Charge",c:"#14b8a6"}].map(f=>{
    ...
    {showAssigned && assigned.length>0 && <DashSection title="Assigned" color="#f59e0b" count={assigned.length}>
...
function OrderEdit({data, db, savOrd, go}) {
  ...
  const saveEvent = async () => {
    ...
    const saveData = {
      ...o,
      orderType:"event",
      status: o.status && o.status!=="unassigned" ? o.status : (hasPrice ? "ready-to-bill" : "unassigned"),
      price:{
        ...(o.price||{}),
        cur: o.price?.cur||"CAD",
...
const STATUS_FLOW = [
  ...
  {s:"assigned", l:"Assigned", c:"#f59e0b"},
...
function StatusChanger({current, onChange, orderType}) {
  ...
  const allowed = STATUS_FLOW.filter(x => {
    ...
    if(orderType === "event") return ["unassigned","ready-to-bill"].includes(x.s) && STATUS_ORDER.indexOf(x.s) < currentIdx;
...
function OrderDetail({o, db, go, setStat, delOrd, savOrd, dupOrd}) {
  ...
  const confirmStatus = async (newStatus, msg, extraFields={}) => {
    ...
    await savOrd({...o, status:newStatus, ...extraFields});
    ...
    <div style={filterBox}>
      ...
      <div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:2}}>
          {[{k:"all",l:"All"},{k:"closed",l:"Closed"},{k:"invoiced",l:"Invoiced"},{k:"ready-to-bill",l:"Ready to Bill"},{k:"in-transit",l:"In Transit"},{k:"assigned",l:"Assigned"},{k:"unassigned",l:"Unassigned"}].map(s=>(
