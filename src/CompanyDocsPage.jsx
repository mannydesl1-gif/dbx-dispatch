...
export default function CompanyDocsPage() {
  ...
  const [docVisibility, setDocVisibility] = useState("all"); // "all" = employees can see, "management" = dispatch only
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setDocName(""); setDocType(""); setDocExpiry(""); setDocFile(null); setDocNotes(""); setDocVisibility("all");
        ...
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, padding:20, marginBottom:20 }}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8, marginTop:6 }}>
              <button type="button" onClick={() => setDocVisibility("all")} style={{
                flex:1, padding:"10px 12px", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:700, transition:"all 0.15s",
                border:`2px solid ${docVisibility==="all" ? T.green : T.border}`,
                background: docVisibility==="all" ? "rgba(34,197,94,0.1)" : "transparent",
                color: docVisibility==="all" ? T.green : T.muted,
              }}>
                👥 All Employees
                <div style={{ fontSize:10, fontWeight:400, marginTop:3, opacity:0.8 }}>Visible in timesheet app</div>
              </button>
              <button type="button" onClick={() => setDocVisibility("management")} style={{
                flex:1, padding:"10px 12px", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:700, transition:"all 0.15s",
                border:`2px solid ${docVisibility==="management" ? T.red : T.border}`,
                background: docVisibility==="management" ? T.redDim : "transparent",
                color: docVisibility==="management" ? T.red : T.muted,
              }}>
                🔒 Management Only
                <div style={{ fontSize:10, fontWeight:400, marginTop:3, opacity:0.8 }}>Dispatch only, hidden from employees</div>
              </button>
            </div>
          </div>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", gap:12, flex:1, minWidth:0 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:4 }}>
                  ...
                  {/* Visibility badge — default "all" for older docs without the field */}
                  {d.visibility === "management"
                    ? <span style={{ fontSize:10, fontWeight:700, color:T.red, background:T.redDim, border:`1px solid ${T.red}`, padding:"2px 8px", borderRadius:6 }}>🔒 Management only</span>
                    : <span style={{ fontSize:10, fontWeight:600, color:T.green, background:"rgba(34,197,94,0.08)", border:`1px solid rgba(34,197,94,0.3)`, padding:"2px 8px", borderRadius:6 }}>👥 All employees</span>
                  }
                </div>
